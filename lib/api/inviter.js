    ...
    sendInvite(options = {}) {
        ...
        this.outgoingInviteRequest = this.userAgent.userAgentCore.invite(this.outgoingRequestMessage, {
            ...
            onProgress: (inviteResponse) => {
                this.logger.log("This SIP.js version contains patch applied by sagar malam to send cancel request when 183 is received if session is canceled already.");
            ...
            onTrying: (inviteResponse) => {
                this.logger.log("This SIP.js version contains patch applied by sagar malam to send cancel request when 100 is received if session is canceled already.");
                // If the user requested cancellation, ignore response.
                if (this.isCanceled) {
                    //This patch is applied by sagar malam to send cancel request when 183 is received if session is canceled already.
                    //This happens when we cancel call before 100 Trying is recieved from the server
